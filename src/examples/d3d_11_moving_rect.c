...
void load(void)
{
	// Init render device
	{
		...
		handle_error(mrl_init_d3d_11_render_device(&desc, &app.rd), u8"mrl_init_d3d_11_render_device() failed");
	}
	...
	// Init vertex shader stage
	{
		...
		desc.src_type = MRL_SHADER_SOURCE_HLSL;
		desc.src =
			"struct vertex_in_t {"
			...
			"vertex_out_t vs(vertex_in_t input) {"
			"	vertex_out_t output;"
	...
	// Init pixel shader stage
	{
		...
		desc.src =
			...
			"cbuffer material_cb { float4 color; };"

			"pixel_out_t ps(pixel_in_t input) {"
			"	pixel_out_t output;"
...
void unload(void)
{
	...
	// Terminate device
	mrl_terminate_d3d_11_render_device(app.rd);
}
